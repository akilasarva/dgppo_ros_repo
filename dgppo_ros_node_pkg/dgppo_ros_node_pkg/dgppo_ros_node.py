...
# import cv2  # Added for homography transformation
# import random  # Added for random choices in transformations
...
#         self.declare_parameter('transform_type', '')  # New parameter for transformation type
#         self.declare_parameter('transform_magnitude', 0.0) # New parameter for transformation magnitude
...
#         self.get_logger().info("To apply transformations: 'ros2 param set /dgppo_ros_node transform_type <type>' 'ros2 param set /dgppo_ros_node transform_magnitude <mag>'")
...
#         transform_type = self.get_parameter('transform_type').get_parameter_value().string_value
#         transform_magnitude = self.get_parameter('transform_magnitude').get_parameter_value().double_value
        
#         # Angular offset is only applied if the transform_type is set to 'angular'
#         angular_offset = 0.0
#         if transform_type == 'angular':
#             angular_offset = transform_magnitude
...
#                     initial_rotation = carla.Rotation(pitch=0, yaw=math.degrees(bearing_rad), roll=0)
#                     # -------------------------------------------------------------
...
#                 new_rotation = carla.Rotation(pitch=0, yaw=math.degrees(bearing_rad), roll=0)
...
#         # The new location calculated by the policy
...
#         # Apply transformations to the new location
#         transformed_location = self._apply_transform_to_location(new_location)

#         # Use the transformed location for the new transform
#         new_transform = carla.Transform(transformed_location, self.ego_vehicle.get_transform().rotation)
...
#         self.get_logger().info(f"Teleporting to X: {transformed_location.x}, Y: {transformed_location.y}")
        
#     def _apply_transform_to_location(self, new_location: carla.Location) -> carla.Location:
#         """
#         Applies a transformation to the target location calculated by the policy.
#         """
#         transform_type = self.get_parameter('transform_type').get_parameter_value().string_value
#         magnitude = self.get_parameter('transform_magnitude').get_parameter_value().double_value

#         if not transform_type:
#             return new_location
        
#         start_point_original = self.ego_vehicle.get_transform().location
#         original_destination = new_location
        
#         self.get_logger().info(f"Applying '{transform_type}' transformation with magnitude {magnitude}")

#         if transform_type == 'roll':
#             # This applies a 2D planar rotation to the vector from start to destination
#             roll_angle_rad = math.radians(magnitude)
#             cos_theta = math.cos(roll_angle_rad)
#             sin_theta = math.sin(roll_angle_rad)
            
#             vector_x = original_destination.x - start_point_original.x
#             vector_y = original_destination.y - start_point_original.y
            
#             new_vector_x = vector_x * cos_theta - vector_y * sin_theta
#             new_vector_y = vector_x * sin_theta + vector_y * cos_theta
            
#             new_location.x = start_point_original.x + new_vector_x
#             new_location.y = start_point_original.y + new_vector_y
#             self.get_logger().info(f"Applying roll: Original destination ({original_destination.x:.2f}, {original_destination.y:.2f}) -> Transformed destination ({new_location.x:.2f}, {new_location.y:.2f})")


#         elif transform_type == 'homography':
#             # Note: This is a hardcoded homography example. For a real application,
#             # these points would need to be dynamically determined.
#             pts_original = np.float32([
#                 [start_point_original.x - 1, start_point_original.y],
#                 [start_point_original.x + 1, start_point_original.y],
#                 [original_destination.x + 1, original_destination.y],
#                 [original_destination.x - 1, original_destination.y]
#             ])
#             pts_skewed = np.float32([
#                 [pts_original[0,0], pts_original[0,1]],
#                 [pts_original[1,0] - magnitude, pts_original[1,1]],
#                 [pts_original[2,0] - magnitude, pts_original[2,1]],
#                 [pts_original[3,0], pts_original[3,1]]
#             ])
#             matrix = cv2.getPerspectiveTransform(pts_original, pts_skewed)
            
#             original_points = np.float32([[
#                 [start_point_original.x, start_point_original.y],
#                 [original_destination.x, original_destination.y]
#             ]])
            
#             transformed_points = cv2.perspectiveTransform(original_points, matrix)[0]
            
#             new_start_x, new_start_y = transformed_points[0]
#             new_end_x, new_end_y = transformed_points[1]

#             new_location.x = new_end_x
#             new_location.y = new_end_y

#             self.get_logger().info(f"Applying homography: Original destination ({original_destination.x:.2f}, {original_destination.y:.2f}) -> Transformed destination ({new_location.x:.2f}, {new_location.y:.2f})")

#         elif transform_type == 'scaling':
#             scaling_factor = magnitude
#             vector_to_destination = carla.Location(x=original_destination.x - start_point_original.x, 
#                                                    y=original_destination.y - start_point_original.y, 
#                                                    z=0.0)
#             scaled_vector = carla.Location(x=scaling_factor * vector_to_destination.x,
#                                            y=scaling_factor * vector_to_destination.y,
#                                            z=0.0)
#             new_location = start_point_original + scaled_vector
            
#         elif transform_type == 'angular':
#             rotation_magnitude = magnitude * random.choice([-1, 1])
#             vector_to_destination = np.array([original_destination.x - start_point_original.x, 
#                                               original_destination.y - start_point_original.y])
#             angle_rad = math.radians(rotation_magnitude)
#             rotation_matrix = np.array([[math.cos(angle_rad), -math.sin(angle_rad)],
#                                         [math.sin(angle_rad), math.cos(angle_rad)]])
#             rotated_vector = np.dot(rotation_matrix, vector_to_destination)
            
#             new_location = carla.Location(x=start_point_original.x + rotated_vector[0],
#                                           y=start_point_original.y + rotated_vector[1],
#                                           z=original_destination.z)

#         elif transform_type == 'positional':
#             translation_x = 0.0
#             translation_y = 0.0
#             if random.choice(['x', 'y']) == 'x':
#                 translation_x = magnitude * random.choice([-1, 1])
#             else:
#                 translation_y = magnitude * random.choice([-1, 1])
            
#             new_location.x += translation_x
#             new_location.y += translation_y
        
#         else:
#             self.get_logger().warn(f"Unknown transform type: '{transform_type}'. No transformation applied.")
...
#         transform_type = self.get_parameter('transform_type').get_parameter_value().string_value
#         if transform_type == 'angular':
#             angular_offset = self.get_parameter('transform_magnitude').get_parameter_value().double_value
...
class DGPPOROSNode(Node):
    ...
    #                 scaled_pos_y_model = (centroid[0] - self.origin_x) / self.scale_2d_3d
    #                 scaled_agent_state_np = np.array([scaled_pos_x_model, scaled_pos_y_model, 0.0, 0.0], dtype=np.float32)
    #                 self.get_logger().info(f"state vals: {scaled_agent_state_np}")
    #                 self.current_agent_state = jnp.expand_dims(jnp.array(scaled_agent_state_np), axis=0)
    #                 initial_location = carla.Location(x=float(centroid[0]), y=float(-1*centroid[1]), z=self.ego_vehicle.get_transform().location.z)
                    
    #                 # Set the initial rotation. Yaw=0 is North, 90 is East, 180 is South, 270 is West.
    #                 initial_rotation = carla.Rotation(pitch=0, yaw=90, roll=0)
...
#     def _load_plan_and_cluster_data(self, model_dir):
#         # Load the plan and centroid data from the JSON file
#         plan_file_path = "plans/highlevel_plan.json"
#         if not os.path.exists(plan_file_path):
#             self.get_logger().error(f"High level plan file not found at {plan_file_path}")
#             return [], {}, {}
...
#             self.get_logger().warning("Waiting for all required data...")
#             return

#         if self.current_plan_step_index >= len(self.plan_sequence):
#             self.get_logger().info("High-level plan is complete. Publishing zero velocity and stopping.")
...
#             return # Skip this loop to give a moment for the new state to be processed
...
#         next_goal_centroid = self.cluster_centroids[next_goal_cluster_id]
        
#         # Calculate bearing to the next goal
#         current_pose_carla = self.latest_agent_state_msg.pose.pose.position
#         goal_pose_carla = next_goal_centroid
#         bearing_value = math.atan2(goal_pose_carla[1] - current_pose_carla.y, goal_pose_carla[0] - current_pose_carla.x)
#         self.get_logger().info(f"Calculated bearing to next goal: {bearing_value:.2f} rad")
        
#         # Update agent state for the RL model
#         if self.current_agent_state is None:
#             # Initialize internal agent state on first run
#             initial_goal_x_carla = next_goal_centroid[0]
#             initial_goal_y_carla = next_goal_centroid[1]
#             scaled_pos_x_model = (initial_goal_y_carla - self.origin_y) / self.scale_2d_3d
#             scaled_pos_y_model = (initial_goal_x_carla - self.origin_x) / self.scale_2d_3d
#             self.current_agent_state = jnp.expand_dims(jnp.array([scaled_pos_x_model, scaled_pos_y_model, 0.0, 0.0], dtype=np.float32), axis=0)

#         # Build graph and get action
#         scaled_ranges_np = np.array(self.latest_ranges_msg.data, dtype=np.float32) / self.scale_2d_3d
#         graph = self._build_state_and_graph(
#             jnp.squeeze(self.current_agent_state),
#             scaled_ranges_np,
#             mapped_current_cluster,
#             expected_start_cluster,
#             expected_next_cluster,
...
#         cmd_vel_msg = Twist()
#         cmd_vel_msg.linear.x = float(action[0, 0])
#         cmd_vel_msg.angular.z = float(action[0, 1])
#         self.cmd_vel_publisher.publish(cmd_vel_msg)
        
#         self.get_logger().info(f"Published control: Throttle={cmd_vel_msg.linear.x:.2f}, Steer={cmd_vel_msg.angular.z:.2f}")
