...
class DGPPOROSNode(Node):
    ...
    def agent_state_callback(self, msg: Odometry):
        ...
        # Extract linear velocities from the twist
        vx = msg.twist.twist.linear.x
        vy = msg.twist.twist.linear.y
        
        # Create the desired list format [x, y, vx, vy]
        robot_state = [x, y, vx, vy]
        
        self.get_logger().info(f'Received state: {robot_state}')
        self.latest_agent_state_msg = robot_state
    ...
    def control_loop(self):
        ...
        if self.is_first_run:
            if self.current_plan_step_index < len(self.plan_sequence):
                ...
                if start_cluster_id in self.cluster_centroids:
                    ...
                    # Get bearing from the plan sequence for the first step
        ...
        if mapped_current_cluster == expected_next_cluster:
            ...
            if self.current_plan_step_index < len(self.plan_sequence):
                ...
                # Get bearing from the new plan step
        ...
        scaled_agent_state = np.array(self.latest_agent_state_msg, dtype=np.float32) / self.scale_2d_3d
        ...
        next_pos_x_carla = next_state_small_frame_squeezed[0] * self.scale_2d_3d + self.origin_x
        next_pos_y_carla = next_state_small_frame_squeezed[1] * self.scale_2d_3d + self.origin_y
        next_vel_x_carla = next_state_small_frame_squeezed[2] * self.scale_2d_3d 
        next_vel_y_carla = next_state_small_frame_squeezed[3] * self.scale_2d_3d 

        self.get_logger().info(f"Action: {action}")
        self.get_logger().info(f"Teleporting to X: {next_pos_x_carla}, Y: {next_pos_y_carla}, VX: {next_vel_x_carla}, VY: {next_vel_y_carla}")
        
        if self.latest_odom_msg is not None:
            orientation_q = self.latest_odom_msg.pose.pose.orientation
            orientation_list = [orientation_q.x, orientation_q.y, orientation_q.z, orientation_q.w]
            
            # Convert the quaternion to Euler angles to get the yaw
            (_, _, current_yaw_rad) = euler_from_quaternion(orientation_list)
            
            # Get the desired bearing from the plan for the *current* step
            current_plan_key = f"{expected_start_cluster}-{expected_next_cluster}"
            desired_bearing_rad = self.bearing_map.get(current_plan_key, current_yaw_rad) # Use current yaw as a fallback
